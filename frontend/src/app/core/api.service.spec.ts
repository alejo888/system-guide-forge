...
describe('ApiService', () => {
  ...
  it('updates a document with only editable fields in array order', async () => { const payload: DocumentUpdatePayload = { title: 'Updated guide', sections: [{ id: 'section-2', title: 'Second', content: 'Content 2' }, { id: 'section-1', title: 'First', content: 'Content 1' }] }; fetchSpy.and.resolveTo(response({})); await service.updateDocument('doc-1', payload); expect(fetchSpy).toHaveBeenCalledWith('/api/documents/doc-1', jasmine.objectContaining({ method: 'PUT', body: JSON.stringify(payload) })); });
