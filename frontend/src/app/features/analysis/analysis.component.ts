...
export class AnalysisComponent implements OnInit {
  ...
  async downloadDocx(): Promise<void> { const current = this.document(); if (!current) return; const blob = await this.api.exportDocument(current.id); const url = URL.createObjectURL(blob); const link = window.document.createElement('a'); link.href = url; link.download = this.docxFilename(current.title); window.document.body.appendChild(link); link.click(); link.remove(); URL.revokeObjectURL(url); }
