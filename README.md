...
El análisis ejecuta el adaptador de forma síncrona, recorre únicamente enlaces de mismo origen clasificados como `SAFE` y nunca ejecuta controles. Las acciones `MUTATING` y `UNKNOWN` quedan bloqueadas. El backend admite profundidad de 0 a 5 (2 si se omite en la API; la interfaz actual envía 0 inicialmente), hasta 100 páginas incluyendo la inicial y un presupuesto global de 500 enlaces evaluados. Las rutas excluidas son prefijos de ruta y también excluyen descendientes. Si cambia una aprobación de inclusión manual, se elimina el borrador existente para que se genere uno nuevo y no se reutilice contenido obsoleto. Si un análisis falla, el registro fallido y cualquier evidencia parcial permanecen disponibles para inspección, marcados como incompletos. Desde ese registro se puede iniciar un análisis nuevo con la configuración actual de la aplicación; no hay recuperación automática, reintento sobre el mismo análisis ni eliminación de la evidencia fallida. La generación de documentos permanece disponible solo para análisis `COMPLETED`.
