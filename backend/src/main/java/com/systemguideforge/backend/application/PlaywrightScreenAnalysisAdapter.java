...
public final class PlaywrightScreenAnalysisAdapter implements ScreenAnalysisAdapter {
    ...
    @Override public ScreenAnalysisResult analyze(TargetApplication application, String username, String password) {
        try (Playwright playwright=browserFactory.create(); Browser browser=playwright.chromium().launch(new BrowserType.LaunchOptions().setHeadless(true)); BrowserContext context=browser.newContext()) {
            ...
            Locator pass=page.locator("input[type='password']").first(); Locator submit=page.locator("button[type='submit'], input[type='submit']").first();
            ...
            if(pass.isVisible()) throw new IllegalStateException("Authentication failed");
            List<DetectedElement> found=new ArrayList<>();
            detect(page,"button",found); detect(page,"a",found); detect(page,"input",found);
            ...
            byte[] screenshot=page.screenshot(new com.microsoft.playwright.Page.ScreenshotOptions()
                    .setFullPage(true)
                    .setMask(List.of(sensitiveFields)));
    ...
    private void detect(com.microsoft.playwright.Page page,String kind,List<DetectedElement> found) {
        ...
        for(int i=0;i<count;i++) { Locator item=locator.nth(i); String selector=kind+":nth-of-type("+(i+1)+")"; String aria=item.getAttribute("aria-label"); String name=item.getAttribute("name"); String placeholder=item.getAttribute("placeholder"); String type=item.getAttribute("type"); String href=item.getAttribute("href"); String label=first(aria,name,placeholder, text(item));
            String attribute= "a".equals(kind) ? "href" : "type"; String value="a".equals(kind) ? href : type;
            found.add(new DetectedElement(kind,selector,safe(label),ActionClassifier.classify(kind,attribute,value)));
        }
    }
    ...
    private static String safe(String value){return value==null?null:value.replaceAll("(?i)password|secret|credential|token", "[redacted]");}
